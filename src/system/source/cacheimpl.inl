...
VDCachedObject *VDCache::Allocate(sint64 key) {
	...
	vdsynchronized(mLock) {
		pObj = AllocateLocked(key);
	}

	return pObj;
}

VDCachedObject *VDCache::AllocateLocked(sint64 key) {
	...
	VDCachedObject *pObj = static_cast<VDCachedObject *>(fl.front());
...
VDCachedObject *VDCache::Create(sint64 key, bool& is_new) {
	...
	vdsynchronized(mLock) {
		...
		if (pObj && pObj->GetState() == kVDCacheStateAborting)
			pObj = NULL;
