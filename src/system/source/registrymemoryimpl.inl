...
void VDRegistryProviderMemory::Value::SetBinary(const void *p, size_t len) {
	...
	if (len)
		mRawData.assign((const char *)p, (const char *)p + len);
	else
		mRawData.clear();
}
...
void VDRegistryProviderMemory::Key::Remove(VDRegistryProviderMemory::Key *key) {
	...
	for(auto it = mKeyMap.begin(), itEnd = mKeyMap.end(); it != itEnd; ++it) {
		if (&it->second == key) {
			auto listIt = std::find(mKeyList.begin(), mKeyList.end(), &*it);
			if (listIt != mKeyList.end())
				mKeyList.erase(listIt);
...
VDRegistryProviderMemory::Key *VDRegistryProviderMemory::Key::OpenKey(const VDStringSpanA& name, bool create) {
	...
	if (it != mKeyMap.end() && !it->second.mbCondemned)
		return &it->second;
	if (it != mKeyMap.end())
		return NULL;
...
bool VDRegistryProviderMemory::GetBinary(void *key0, const char *name, char *buf, int maxlen) {
	vdsynchronized(mMutex) {
		...
		const void *p;
		size_t len;
		if (!value || !value->GetBinary(p, len) || (int)len > maxlen || (len && !buf))
			return false;
