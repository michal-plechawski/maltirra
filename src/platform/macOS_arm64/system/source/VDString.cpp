...
VDStringA& VDStringA::append_vsprintf(const value_type *format, va_list val) {
	...
	va_list copy;
	va_copy(copy, val);
	int len = vdvsnprintf(buf, 2048, format, copy);
	va_end(copy);
	...
	if (len >= 0 && len < 2048)
		...
		int siz = len < 0 ? 8192 : len < 65536 ? len + 1 : 65537;
		...
		while(siz <= 65536) {
			...
			char *tmpp = tmp.data();
			va_copy(copy, val);
			len = vdvsnprintf(tmpp, siz, format, copy);
			va_end(copy);
...
VDStringW& VDStringW::append_vsprintf(const value_type *format, va_list val) {
	...
	va_list copy;
	va_copy(copy, val);
	int len = vdvswprintf(buf, 1024, format, copy);
	va_end(copy);
	...
	if (len >= 0 && len < 1024)
		...
		int siz = len < 0 ? 4096 : len < 65536 ? len + 1 : 65537;
		...
		while(siz <= 65536) {
			...
			va_copy(copy, val);
			len = vdvswprintf(tmpp, siz, format, copy);
			va_end(copy);
